...
export default function MonthlyDashboardPopup({ tasks, onClose, isStandalone = false }: Props) {
  ...
  const [selectedMonth, setSelectedMonth] = useState<number | null>(null); // 🌟 월별 필터 상태 추가
  ...
  }, [tasks, targetDate, selectedMonth]); // 🌟 종속성에 selectedMonth 추가
                  ...
                  return (
                    <div className="flex items-center justify-between px-5 py-3 bg-indigo-50/30 border-b border-indigo-100 shrink-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[14px] font-extrabold text-indigo-900">{stat.area} 진행 현황 요약 {selectedMonth && `(${selectedMonth}월 누적)`}</span>
                      </div>
