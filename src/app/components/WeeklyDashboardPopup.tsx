...
export default function WeeklyDashboardPopup({ tasks, onClose }: Props) {
  ...
  const { overallPlanned, overallActual, areaStats, detailedTasks, totalValidCount, completedCount, incompleteCount } = useMemo(() => {
    ...
    // 🌟 정렬: 지연항목 우선(1순위) -> 산출물 내림차순(2순위) -> 실제진행률 오름차순(3순위)
    details.sort((a, b) => {
      const isDelayedA = a.actualProg < a.plannedProg ? 1 : 0;
      const isDelayedB = b.actualProg < b.plannedProg ? 1 : 0;
      if (isDelayedA !== isDelayedB) return isDelayedB - isDelayedA; // 지연 항목 먼저
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
      {/* 🌟 가로 사이즈 대폭 확대 */}
      <div className="bg-white rounded-xl shadow-2xl w-[1200px] h-[85vh] max-w-[95vw] flex flex-col overflow-hidden relative border border-slate-200 font-sans">
